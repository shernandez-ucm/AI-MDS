...
\end{center}
\vspace{0.1em}
\idea{$P \rightarrow Q$ \textbf{no} afirma causalidad: solo excluye el caso $P$ verdadero con $Q$ falso. \emph{Si gano la elección, bajaré los impuestos} es una proposici\'on verdadera.}
\end{frame}
...
\vspace{0.4em}
\footnotesize
Si sabemos que \emph{si llueve, entonces la calle está mojada} y adem\'as \emph{llueve}, podemos afirmar \emph{la calle está mojada}.
\end{column}
\begin{column}{6.4cm}
...
\end{columns}
\vspace{-0.2em}
\idea{Los factores de certeza fueron una soluci\'on \emph{de ingenier\'ia}, no probabil\'istica: no satisfacen los axiomas de Kolmogorov. Las \textbf{redes bayesianas},  son la respuesta formal al mismo problema.}
\end{frame}
...
\footnotesize
Modelar el dominio con una \textbf{distribución conjunta}. Con $A$ la enfermedad y $B$ los síntomas, toda pregunta es marginalizar y condicionar:
\vspace{-0.4em}
\[
...
\end{center}
\footnotesize
\textbf{Dirigidos --- Bayesian Networks}\\
...
\end{center}
\footnotesize
\textbf{No dirigidos --- Markov Random Fields}\\
