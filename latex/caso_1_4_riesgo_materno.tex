...
\newcommand{\enlace}[1]{\par\vspace{2pt}{\footnotesize\color{StanfordGray}%
	$\hookrightarrow$\ \textit{Clase 1.4:} #1}}
...
{\Large\bfseries ¿Por qué (no) funciona el aprendizaje profundo\\ en la detección de riesgo materno?}\\[10pt]
{\normalsize MDS221 --- Inteligencia Artificial \quad$\bullet$\quad Unidad 1, clase 1.4}\\[2pt]
...
% =====================================================================
\section{El resultado que hay que explicar}
...
\vspace{0.8em}
\noindent
Las dos arquitecturas neuronales quedan \textbf{\'ultimas entre los m\'etodos no triviales}, quince puntos por debajo del bosque. Ese es el hecho a explicar. La clase 1.4 entrega el vocabulario para hacerlo: sobreparametrizaci\'on, umbral de interpolaci\'on, doble descenso, planitud del m\'inimo, sesgo inductivo, billete de loter\'ia. El trabajo consiste en \textbf{usar ese vocabulario como instrumento de medici\'on}, no como decorado.
...
\subsection*{T2 --- ¿Puede la red memorizar este problema?}
\begin{enumerate}
...
\subsection*{T3 --- Barrido de capacidad: ¿aparece el doble descenso?}
\begin{enumerate}
	...
	\item Si \textbf{no} aparece el doble descenso, no lo fuerce: explique qu\'e condiciones de la clase 1.4 no se cumplen aqu\'i (tama\~no de $n$, ruido de etiqueta, regularizaci\'on impl\'icita del optimizador).
\end{enumerate}
\enlace{l\'amina ``Sobreparametrizaci\'on y doble descenso''. Recuerde: la curva en U cl\'asica no est\'a mal, est\'a \emph{incompleta} --- pero solo cuando se llega al r\'egimen moderno.}

\subsection*{T4 --- Razón lote/tasa y planitud del mínimo}
\begin{enumerate}
...
\subsection*{T5 --- Billete de lotería a escala pequeña}
\begin{enumerate}
	\item Entrene su mejor MLP y guarde la inicializaci\'on. Pode por magnitud hasta conservar el 20\,\%, 10\,\% y 5\,\% de los pesos.
	\item Reentrene dos veces cada subred: (a) desde \textbf{la misma} inicializaci\'on original, (b) desde una inicializaci\'on aleatoria nueva.
	\item ¿Se reproduce el resultado de Frankle \& Carbin a esta escala? Si la diferencia entre (a) y (b) desaparece, discuta si es porque el modelo es demasiado peque\~no para contener ``billetes ganadores''.
\end{enumerate}
\enlace{l\'aminas ``Activaci\'on, inicializaci\'on y profundidad'' y ``Poda''.}

\subsection*{T6 --- Sesgo inductivo: por qué gana el bosque}
\begin{enumerate}
...
\begin{enumerate}[leftmargin=1.6em]
	\item \textbf{Cuaderno reproducible} (\texttt{.ipynb}). Debe correr de principio a fin sin intervenci\'on manual, con semillas fijadas y la descarga del dato incluida. Un cuaderno que no corre se eval\'ua como si no se hubiera entregado.
...
\begin{aviso}
\textbf{Un resultado negativo bien establecido vale tanto como uno positivo.} Si tras un barrido honesto la red sigue perdiendo contra el bosque, esa \emph{es} la conclusi\'on: el cap\'itulo 20 sostiene que la sobreparametrizaci\'on ayuda \textbf{a la escala y complejidad de los conjuntos que se usan hoy en visi\'on y lenguaje}, no que una red gane siempre. Lo que no se acepta es afirmar sin medir.
\end{aviso}
